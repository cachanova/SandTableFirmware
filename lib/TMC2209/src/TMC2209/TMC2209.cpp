  ...
  SerialAddress serial_address)
{
  ...
  // Set the address without changing GCONF, then command zero current before
  // switching I_SCALE_ANALOG off. At reset the chip can have IRUN=31 and
  // TOFF=3; changing to the internal reference first would expose that stale
  // current setting for one UART datagram interval.
...
{
  // The upstream defaults wrote IRUN=31 and then enabled CHOPCONF.TOFF=3
  // before initialize() minimized current and disabled the bridge. On boards
  // with ENN tied low, that ordering creates a high-current pulse at every
  // firmware boot. Seed the caches with the same defaults but write zero
  // current and TOFF=0 from the first two motor-control datagrams.
  driver_current_.bytes = 0;
  driver_current_.ihold = CURRENT_SETTING_MIN;
  driver_current_.irun = CURRENT_SETTING_MIN;
  driver_current_.iholddelay = IHOLDDELAY_DEFAULT;
  ...
  uint8_t datagram_size)
{
  ...
  // This project uses a single-wire UART adapter whose transmitted bytes are
  // echoed into the ESP32 RX path. Upstream leaves those bytes queued after
  // every register write, so a sequence of writes can fill the RX FIFO and
  // corrupt the next checked transaction. Consume the echo after each write.
  if (hardware_serial_ptr_ != nullptr)
  {
    serialFlush();
